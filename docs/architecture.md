# Blockwright — architecture deep-dive

> The full subsystem documentation. `CLAUDE.md` (repo root) is the slim always-loaded map;
> read the relevant section HERE before non-trivial work on a subsystem (AI generation,
> world viewer/editor, jigsaw split, export, block editor, domain modules, updates, theming/icons).
...
src/
  ...
  renderer/                React app (Vite + @vitejs/plugin-react). No Node/fs/electron — IPC only.
    ...
    components/           ActivityBar (the left icon rail: House=Home on top, Project/New-build/Catalog/
                          ...
                          its structures searchable + its JIGSAW template pools grouped by folder — the pool
                          FAMILY — each expandable to its resolved pieces (`listWorkspaceJigsaws`, dead refs
                          warn-tinted, click opens the piece) + recent files/workspaces/worlds; toggled from the rail
    ...
    world/                The World Viewer's RENDERER side: stream a Minecraft world's chunks around the
                          ...
                          edit-overlay.ts      IN-WORLD EDITING (v2.2 §2), the pure compositor: pending
                                               ...
                                               PLACE (v2.2 §3: an open structure doc as a translucent
                                               ghost — world/place.ts is the pure math: rotateCell ⇄
                                               ghostTransform agree exactly, props via transformProps,
                                               air clears / void+omitted preserve; viewer/world-ghost.ts
                                               renders it; commit → pending edits, one undo step),
